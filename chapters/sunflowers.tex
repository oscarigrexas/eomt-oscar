...
The method itself consists in the optimization of the coefficients of a multiple linear regression model.\sidenote{
	\begin{equation}
        \labeq{multiple-linear-regression}
        y_i \approx \beta_0 + \beta_1 x_{i1} + \beta_2 x_{i2} + ... + \beta_p x_{ip} \quad i=1,...,n
	\end{equation}
}%
The bits of real data that we have -our ground truth- are the coordinates of the atoms of the molecule, which include $x$, $y$ and $z$ values.
...
There is just a problem with this approach: $x$ and $y$ by themselves are not enough to generate the kind of warped, ondulated surfaces that we're looking for! Just by adding different proportions of $x$ and $y$, the best one could get would be a tilted plane.

We wish to find out the values of the coefficients for each of them.
